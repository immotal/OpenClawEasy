...
export function buildPosixWrapperForPaths(nodeBin: string, entry: string): string {
  ...
  return [
    ...
    `APP_ENTRY="${safeEntry}"`,
    'if [ ! -f "$APP_NODE" ]; then',
    '  echo "Error: OpenClaw not found at $APP_NODE" >&2',
    "  exit 127",
    "fi",
    'if [ ! -f "$APP_ENTRY" ]; then',
    '  echo "Error: OpenClaw entry not found at $APP_ENTRY" >&2',
    "  exit 127",
    "fi",
...
export function buildWinWrapperForPaths(nodeBin: string, entry: string): string {
  ...
  return [
    ...
    "  echo Error: OpenClaw Node runtime not found. 1>&2",
    "  exit /b 127",
    ")",
    'if not exist "%APP_ENTRY%" (',
    "  echo Error: OpenClaw entry not found. 1>&2",
    "  exit /b 127",
    ")",
